...
#error "Notification needs to be done ! Unconditionally !"
        ...
        case EC_NOTIFY_SB_EEPROM_ACCESS_DENIED:
            ...
            EC_T_SLAVE_PROP* pSlavePropParent = &pNotificationDesc->desc.EEPROMAccessDeniedDesc.SlavePropParent;
                ...
                pSlaveProp->wAutoIncAddr, pSlavePropParent->achName, pSlavePropParent->wStationAddress, pNotificationDesc->desc.EEPROMAccessDeniedDesc.wPortAtParent));

            } break;
        case EC_NOTIFY_SB_SLAVE_IDENT_TIMEOUT:
            {
                EC_T_SLAVE_PROP* pSlaveProp = &pNotificationDesc->desc.SlaveIdentTimeoutDesc.SlaveProp;
                EC_T_SLAVE_PROP* pSlavePropParent = &pNotificationDesc->desc.SlaveIdentTimeoutDesc.SlavePropParent;
                EcLogMsg(EC_LOG_LEVEL_ERROR, (pEcLogContext, EC_LOG_LEVEL_ERROR, GetText(EC_TXT_SB_SLAVE_IDENT_TIMEOUT), pSlaveProp->achName, pSlaveProp->wStationAddress,
                    pSlaveProp->wAutoIncAddr, pSlavePropParent->achName, pSlavePropParent->wStationAddress, pNotificationDesc->desc.SlaveIdentTimeoutDesc.wPortAtParent));
            } break;
            /**********************/
            /* error notification */
            /**********************/
        case EC_NOTIFY_CYCCMD_WKC_ERROR:    /* ERR|1 */
            {
                ...
                pErrorNotificationDesc->desc.WkcErrDesc.dwTaskId,
                pErrorNotificationDesc->desc.WkcErrDesc.wMsuId));
            } break;
    ...
    case EC_NOTIFY_CONFIG_LOAD:
        {
            EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, GetText(EC_TXT_CONFIG_LOAD)));
        } break;
    default:
        {
...
#if (defined NO_OS)
    ((CAtEmLogging*)pEcLogParms->pLogContext)->ProcessAllMsgs();
#endif
    ...
    case ECMASTERRAS_NOTIFY_CONNECTION:         /* GENERIC RAS | 1 */
        {
            ...
            {
                EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "RAS client connected\n"));
                m_bRasServerDisconnect = EC_FALSE;
            }
            else
            {
                EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "RAS connection changed: %s (0x%08X)\n", GetText(pConNot->dwCause), pConNot->dwCause));
