...
/* 从外部（BasicService）注入的电机配置
 * g_MotorSlaveAddr[i]  = ENI 中第 i 个电机的 PhysAddr（站地址）
 * g_MotorDdsIndex[i]   = 该电机对应全身第几号电机（DDS 索引，0-indexed）
 * g_MotorCount         = 电机数量
 */
static EC_T_WORD g_MotorSlaveAddr[MAX_SLAVE_NUM];
static int       g_MotorDdsIndex[MAX_SLAVE_NUM];
static int       g_MotorCount = 0;
static char      g_CalibFilePath[512] = "../../../conf/cfg.json"; /* 默认相对路径 */

void EcDemo_SetMotorMap(const int* map, int count)
{
    /* map 只含 dds_index，slave_addr 需另外设置 —— 见 EcDemo_SetMotorConfig */
    (void)map; (void)count;
}

/* 完整配置接口：同时设置站地址和 DDS 索引映射
 * slave_addrs[i] = 第 i 个电机的 ENI PhysAddr
 * dds_indices[i] = 第 i 个电机的 DDS 全身索引（0-indexed）
 */
void EcDemo_SetMotorConfig(const EC_T_WORD* slave_addrs, const int* dds_indices, int count,
                           const char* calib_file)
{
    g_MotorCount = count < MAX_SLAVE_NUM ? count : MAX_SLAVE_NUM;
    for (int i = 0; i < g_MotorCount; i++) {
        g_MotorSlaveAddr[i] = slave_addrs[i];
        g_MotorDdsIndex[i]  = dds_indices[i];
    }
    MT_SetMotorMap(dds_indices, g_MotorCount);
    if (calib_file && calib_file[0]) {
        OsSnprintf(g_CalibFilePath, sizeof(g_CalibFilePath) - 1, "%s", calib_file);
    }
}
...
#define CALIB_FILE_NAME g_CalibFilePath
#define TOTAL_JOINTS 28  // 全部电机28个
...
static bool LoadCalibrationFile(const char* filename)
{
    ...
    // 读取关节总数
    // 若 cfg 未提供 total_joints：优先用 busi.yaml motor_map 的电机数（g_MotorCount），一致只需改一处
    ...
    if (g_CalibData.total_joints <= 0 || g_CalibData.total_joints > TOTAL_JOINTS) {
        g_CalibData.total_joints = (g_MotorCount > 0 ? g_MotorCount : TOTAL_JOINTS);
    }
...
EC_T_DWORD EcDemoApp(T_EC_DEMO_APP_CONTEXT* pAppContext)
{
    ...
    /* 当由 BasicService 等直接调用 EcDemoApp（不经过 EcDemoMain）时，pvJobTaskEvent 未被设置，
     * JobTask 内 OsWaitForEvent(pvJobTaskEvent) 会空指针解引用导致段错误。此处若尚未创建 TimingTask 则创建并启动。 */
    CDemoTimingTaskPlatform* pOwnTimingTask  = EC_NULL;
#endif
     ...
     * - 若由 BasicService 直接调用（未经过 EcDemoMain），此处尚未启动 TimingTask，需先创建并启动
     */
#if (defined EXECUTE_DEMOTIMINGTASK)
    if (EC_NULL == pAppContext->pvJobTaskEvent)
    {
        pOwnTimingTask = EC_NEW(CDemoTimingTaskPlatform(*pAppContext));
        if (EC_NULL == pOwnTimingTask)
        {
            dwRetVal = EC_E_NOMEMORY;
            EcLogMsg(EC_LOG_LEVEL_ERROR, (pEcLogContext, EC_LOG_LEVEL_ERROR, "ERROR: Cannot create timing task (embedded EcDemoApp)\n"));
            goto Exit;
        }
        dwRes = pOwnTimingTask->StartTimingTask(pAppParms->dwBusCycleTimeUsec * 1000);
        if (EC_E_NOERROR != dwRes)
        {
            dwRetVal = dwRes;
            EcLogMsg(EC_LOG_LEVEL_ERROR, (pEcLogContext, EC_LOG_LEVEL_ERROR, "ERROR: Cannot start timing task: %s (0x%lx)\n", ecatGetText(dwRes), dwRes));
            SafeDelete(pOwnTimingTask);
            pOwnTimingTask = EC_NULL;
            goto Exit;
        }
    }
#endif
...
#if (defined EXECUTE_DEMOTIMINGTASK)
    /* 若由 EcDemoApp 内部创建的 TimingTask，此处停止并释放（BasicService 嵌入调用时） */
    if (EC_NULL != pOwnTimingTask)
    {
        pOwnTimingTask->StopTimingTask();
        SafeDelete(pOwnTimingTask);
        pAppContext->pTimingTaskContext = EC_NULL;
    }
#endif
...
static EC_T_DWORD myAppPrepare(T_EC_DEMO_APP_CONTEXT* pAppContext)
{
    ...
    {
        if (g_MotorCount > 0) {
            /* 使用外部注入的电机配置（来自 busi.yaml motor_map） */
            for (int i = 0; i < g_MotorCount; i++) {
                My_Slave[i].wStationAddress = g_MotorSlaveAddr[i];
                My_Slave[i].wAxisCnt = 1;
            }
        } else {
            /* 回退：14 个电机，站号从 1002 到 1015 */
            for (int i = 0; i < 14; i++) {
...
static void* CmdThread(void*)
{
    ...
    printf("╚══════════════════════════════════════════════════════════════════╝\n");
    printf("\n");
    /* 进入调试模式时若尚未加载标定，按 motor_map 数量自动加载（total_joints 默认 = g_MotorCount） */
    if (!g_CalibData.loaded) {
        (void)LoadCalibrationFile(CALIB_FILE_NAME);
    }
    printf("[CMD] === 紧急命令 ===\n");
    ...
    printf("  setzero                                     当前位置设为零点 (SDO 0x3801=5)\n");
    printf("  calib                                       标定\n");
    ...
    while (fgets(line, sizeof(line), stdin) != nullptr) {
        ...
        if (strcmp(line, "help") == 0 || strcmp(line, "h") == 0 || strcmp(line, "?") == 0) {
            ...
            printf("║   setzero                 当前位置设为零点 (SDO 0x3801=5)，不运动  ║\n");
        ...
        if (strcmp(line, "setzero") == 0) {
            printf("\n╔══════════════════════════════════════════════════════════════════╗\n");
            printf("║              [置零] 将当前位置设为零点 (SDO 0x3801=5)              ║\n");
            printf("╠══════════════════════════════════════════════════════════════════╣\n");

            if (!g_CalibData.loaded) {
                printf("║ 错误: 未加载标定数据！请先确保 cfg.json 存在                          ║\n");
                printf("╚══════════════════════════════════════════════════════════════════╝\n\n");
                fflush(stdout);
                continue;
            }
            ...
            OsSleep(500);
            printf("[setzero] 完成: %d/%d 轴置零成功，已锁定零点\n", success_count, nAxes);
            fflush(stdout);
            continue;
        }
